...
fn misc_8() {
    ...
    assert_eq!(mem.su_get(grr(&bc)), 0xff);
    inc_arr(&mut af, &mut mem, &bc);
    assert_eq!(mem.su_get(grr(&bc)), 0);
...
fn cb_rot() {
    ...
    assert_eq!(mem.su_get(grr(&hl)), 0xd5);
    assert!(!gf((&af2, CY)));
...
fn sh_bit() {
    ...
    assert!(gf((&af, CY)));
    assert_eq!(mem.su_get(grr(&hl)), 0x54);
    assert!(gf((&af2, CY)));
    sla_r(&mut af, (&mut bc, U));
    sla_arr(&mut af2, &mut mem, &hl);
    ...
    bit_msk_n(&mut af, 0x80, gr((&bc, U)));
    assert!(gf((&af, Z)));
    bit_msk_n(&mut af, 0x80, mem.su_get(grr(&hl)));
    assert!(gf((&af, Z)));
